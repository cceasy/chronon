...
object DataFrameGen {
  ...
  def gen(spark: SparkSession,
    ...
    val effectiveColumn = partitionColumn.getOrElse(tableUtils.partitionSpec.column)
    val effectiveFormat = partitionFormat.getOrElse(tableUtils.partitionSpec.format)
    val effectiveSpec = tableUtils.partitionSpec.copy(
